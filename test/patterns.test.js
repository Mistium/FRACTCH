...
const packedBlocks = (manifest, name) => {
  ...
  return Object.fromEntries(
    Object.entries(t.blocks || {}).filter(
      ([, b]) => b && typeof b === 'object' && !Array.isArray(b) && typeof b.opcode === 'string'
    )
  );
};
...
test('obscured shadow: active reporter survives, shadow value is regenerable', async () => {
  ...
  assert.ok(
    Object.values(packed).some((b) => b.opcode === 'operator_add'),
    'plugged reporter survived'
  );
});
    ...
    projectOf(
      target('main', {
        ...
        key: {
          ...
          next: null,
          parent: 'wait',
          inputs: { KEY_OPTION: [3, null, 'menu1'] },
          fields: {},
        },
        menu1: {
...
test('empty if condition still parses and keeps the if block', async () => {
  ...
  const packed = packedBlocks(manifest, 'main');
  assert.ok(
    Object.values(packed).some((b) => b.opcode === 'control_if'),
    'if survived'
  );
  assert.ok(
    Object.values(packed).some((b) => b.opcode === 'looks_say'),
    'body survived'
  );
});
...
test('empty not <> keeps the operator_not block', async () => {
    ...
    projectOf(
      target('main', {
        ...
        iff: {
          ...
          next: null,
          parent: 'hat',
          inputs: { CONDITION: [2, 'not1'], SUBSTACK: [2, 'say'] },
          fields: {},
        },
  ...
  const packed = packedBlocks(manifest, 'main');
  assert.ok(
    Object.values(packed).some((b) => b.opcode === 'operator_not'),
    'operator_not survived'
  );
});
    ...
    projectOf(
      target(
        'main',
        {
          hat: hat('ext'),
          ext: {
            opcode: 'lmsMoreEvents_broadcastData',
            next: null,
            parent: 'hat',
            inputs: {
...
test('same proccode in two sprites with different argument ids', async () => {
  const spriteBlocks = (argId) => ({
    ...
    proto: {
      ...
      mutation: {
        ...
        children: [],
        proccode: 'do thing %s',
        argumentids: JSON.stringify([argId]),
        argumentnames: '["x"]',
        argumentdefaults: '[""]',
        warp: 'false',
      },
  ...
  const manifest = await roundtrip(
    projectOf(target('spriteA', spriteBlocks('arg-a')), target('spriteB', spriteBlocks('arg-b')))
  );
    ...
    projectOf(
      target('main', {
        ...
        call: {
          ...
          next: null,
          parent: 'hat',
          inputs: { 'stale-arg': [1, [10, '42']] },
          fields: {},
          mutation: {
            tagName: 'mutation',
            children: [],
            proccode: 'sized %s',
            argumentids: '["stale-arg"]',
            warp: 'false',
          },
        },
      })
...
test('variable named text: .letter() resolves to operator_letter_of', async () => {
    ...
    projectOf(
      ...
      target('main', {
        ...
        letter: {
          ...
          next: null,
          parent: 'say',
          inputs: { LETTER: [1, [6, '1']], STRING: [3, [12, 'text', 'vtext'], [10, '']] },
          fields: {},
        },
      })
    )
  ...
  const packed = packedBlocks(manifest, 'main');
  assert.ok(
    Object.values(packed).some((b) => b.opcode === 'operator_letter_of'),
    'letter block survived'
  );
...
test('floating shadow is dropped, never re-emitted', async () => {
  ...
  assert.ok(
    !Object.values(packed).some((b) => b.opcode === 'math_number' && b.fields.NUM?.[0] === '9'),
    'floater gone'
  );
});
...
test('switch/case with fallthrough and default round-trips', async () => {
    ...
    projectOf(
      target('main', {
        ...
        sw: {
          ...
          next: null,
          parent: 'hat',
          inputs: { VALUE: [1, [10, 'a']], SUBSTACK: [2, 'c1'] },
          fields: {},
        },
        c1: {
          ...
          next: 'c2',
          parent: 'sw',
          inputs: { VALUE: [1, [10, 'a']], SUBSTACK: [2, 'say1'] },
          fields: {},
        },
        ...
        c2: {
          ...
          next: 'c3',
          parent: 'sw',
          inputs: { VALUE: [1, [10, 'b']], SUBSTACK: [2, 'say2'] },
          fields: {},
        },
  ...
  for (const opcode of ['control_switch', 'control_case_fallthrough', 'control_case', 'control_default']) {
    assert.ok(
      Object.values(packed).some((b) => b.opcode === opcode),
      `${opcode} survived`
    );
  }
});
...
test('workspace and block comments round-trip', async () => {
  ...
  const texts = Object.values(packed.comments || {})
    .map((c) => c.text)
    .sort();
...
test('variable and list monitors with sliders round-trip', async () => {
  const outDir = tmp();
  try {
    const project = projectOf(target('Stage', {}, { variables: { v1: ['score', '0'] }, lists: { l1: ['queue', []] } }));
    project.monitors = [
      {
        id: 'v1',
        mode: 'slider',
...
test('literal fidelity: leading-zero, exponent, empty and unicode strings', async () => {
  ...
  values.forEach((v, i) => {
    blocks[`s${i}`] = {
      opcode: 'looks_say',
      next: i + 1 < values.length ? `s${i + 1}` : null,
      parent: i === 0 ? 'hat' : `s${i - 1}`,
      inputs: { MESSAGE: [1, [10, v]] },
      fields: {},
    };
  });
  const manifest = await roundtrip(projectOf(target('main', blocks)));
  const packed = packedBlocks(manifest, 'main');
  const got = Object.values(packed)
    .filter((b) => b.opcode === 'looks_say')
    .map((b) => String(b.inputs.MESSAGE[1][1]));
    ...
    projectOf(
      target('main', {
        ...
        iff: {
          ...
          next: null,
          parent: 'hat',
          inputs: { CONDITION: [2, 'eq'], SUBSTACK: [2, 'say'] },
          fields: {},
        },
        eq: {

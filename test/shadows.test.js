...
const minimalProject = (blocks) => ({
  targets: [
    { isStage: true, name: 'Stage', blocks: {}, variables: {}, lists: {}, broadcasts: {}, comments: {}, costumes: [], sounds: [] },
    { isStage: false, name: 'main', blocks, variables: {}, lists: {}, broadcasts: {}, comments: {}, costumes: [], sounds: [] },
  ],
  monitors: [],
  extensions: [],
  meta: {},
...
test('convert drops floating shadows instead of emitting them as scripts', async () => {
  const outDir = tmp();
  const blocks = {
    hat: { opcode: 'event_whenflagclicked', next: 'move', parent: null, inputs: {}, fields: {}, topLevel: true, x: 0, y: 0 },
    move: { opcode: 'motion_movesteps', next: null, parent: 'hat', inputs: { STEPS: [3, 'plugged', 'obscured'] }, fields: {} },
    plugged: { opcode: 'operator_add', next: null, parent: 'move', inputs: { NUM1: [1, [4, '1']], NUM2: [1, [4, '2']] }, fields: {} },
    obscured: { opcode: 'math_number', next: null, parent: 'move', inputs: {}, fields: { NUM: ['10', null] }, shadow: true },
    floating: { opcode: 'math_number', next: null, parent: null, inputs: {}, fields: { NUM: ['42', null] }, shadow: true, topLevel: true, x: 5, y: 5 },
    orphanMenu: { opcode: 'sensing_keyoptions', next: null, parent: null, inputs: {}, fields: { KEY_OPTION: ['space', null] }, shadow: true },
  };
...
test('check reports unknown builtin opcodes and menus at statement position', async () => {
  ...
  fs.writeFileSync(path.join(buildDir, 'main', 'main.fractch'), [
    'when flag at 0,0 {',
    '  looks.sayy("hi");',
  ...
  assert.ok(messages.some((m) => m.startsWith('2:') && m.includes("unknown block 'looks.sayy'") && m.includes('looks.say')), `typo flagged with suggestion: ${messages}`);
  assert.ok(messages.some((m) => m.startsWith('3:') && m.includes('dropdown menu')), `statement menu flagged: ${messages}`);
